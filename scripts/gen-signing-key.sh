...
# Generate an Ed25519 signing keypair for pitype release signing.
#
...
# in the installer (netinstall.sh).
#
...
OUT_DIR="${1:-$HOME/.pitype/signing}"

usage() {
    cat <<EOF
Usage: $0 [OUT_DIR]

Generate an Ed25519 keypair for signing pitype releases.

  OUT_DIR  Directory to write the keys (default: \$HOME/.pitype/signing)
...
in the netinstaller (or the pitype-signing-key.pub file).
EOF
}
