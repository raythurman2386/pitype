...
# Install pitype from a prebuilt GitHub Release tarball.
#
...
RELEASE_BASE_URL="${PITYPE_RELEASE_BASE_URL:-$DEFAULT_RELEASE_BASE_URL}"
VERSION=""
...
MCowBQYDK2VwAyEAm5TIOV89CPlLcG+TNyqZ0+6iLHN1Ne6ZoaUeUhE2Rqg=
-----END PUBLIC KEY-----'
...
Environment:
  PITYPE_RELEASE_BASE_URL  Override the release artifact base URL
...
main() {
    ...
    local tarball="pitype-${version_no_v}-${triple}.tar.gz"

    echo "==> Platform: $triple"
    echo "==> Version:  $version_tag"
    echo "==> Prefix:   $PREFIX"

    if [[ -x "$PREFIX/bin/pitype" ]] && [[ "$FORCE" != true ]]; then
        echo "==> pitype is already installed at $PREFIX/bin/pitype"
    ...
    # checksums (and therefore the tarball) were produced by the pitype
    ...
    local pubkey_file
    pubkey_file="$tmp_dir/pitype-signing-key.pub"
    ...
    local package_dir
    package_dir="$(find "$extract_dir" -maxdepth 1 -type d -name 'pitype-'"$version_no_v"'-*' | head -1)"
    ...
    echo "==> Installed pitype $version_tag into $PREFIX"
}
