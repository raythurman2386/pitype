...
# mode (PITYPE_RELEASE_BASE_URL pointing at a filesystem path). Verifies that:
...
TARBALL="pitype-${VERSION}-${TRIPLE}.tar.gz"
...
make_fake_tarball() {
    local stage="$WORK/stage/pitype-${VERSION}-${TRIPLE}"
    rm -rf "$WORK/stage"
    mkdir -p "$stage"
...
mkdir -p "$PREFIX/bin"
printf '#!/usr/bin/env bash\necho "pitype %s (fake)"\n' "$(basename "$0")" > "$PREFIX/bin/pitype"
chmod +x "$PREFIX/bin/pitype"
EOF
    chmod +x "$stage/install.sh"
    tar -czf "$RELEASE_DIR/$TARBALL" -C "$WORK/stage" "pitype-${VERSION}-${TRIPLE}"
}
...
run_install() {
    ...
    PITYPE_RELEASE_BASE_URL="$WORK/release" \
...
[[ -x "$INSTALL_DIR/bin/pitype" ]] || fail "pitype binary not installed"
...
[[ ! -e "$INSTALL_DIR/bin/pitype" ]] || fail "tampered tarball must not be installed"
...
[[ ! -e "$INSTALL_DIR/bin/pitype" ]] || fail "tarball must not be installed on bad signature"
...
[[ ! -e "$INSTALL_DIR/bin/pitype" ]] || fail "tarball must not be installed without a signature"
